...
BOOST_AUTO_TEST_CASE(test_userAgent) {
    ...
    const std::string versionMessage =
        "/Vuhn Bitcoin Cash Node:" + std::to_string(CLIENT_VERSION_MAJOR) + "." +
    ...
    const std::string versionMessage_strprintf =
        strprintf("/Vuhn Bitcoin Cash Node:%d.%d.%d(EB8.0; %s)/", CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION, uacomment);
