...
class P2SH32Test(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.priv_key = ECKey()
        self.priv_key.set(secret=b'P2SH32!!' * 4, compressed=True)
        # Make p2sh32 wrapping p2pkh
        pub_key = self.priv_key.get_pubkey().get_bytes()
        ...
        self.addr_legacy = address.script_to_p2sh32(redeem_script)
        self.log.info(f"Cashaddr: {self.addr_cashaddr}  Legacy: {self.addr_legacy}")
        ...
        self.send_blocks([block], success=False, reject_reason='blk-bad-inputs')
        ...
        blockhashes += node.generatetoaddress(1, self.addr_legacy)  # This time specify the same script using legacy
        assert_equal(blockhashes[-1], node.getbestblockhash())
        ...
        # Sanity check on legacy vs cashaddr encoding for p2sh32
